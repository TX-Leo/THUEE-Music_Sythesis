...
for i = 1 : 7
    if (i==1)
        tunes = zeros([7 4]);
        tunes(6, 1) = 220;                                          %f major-> 1 corresponding to f
        tunes(6, 2) = 440;
        tunes(6, 3) = 880;
        frequency_diff = 2^(1/12).^[-4, -2, 0, 2, 3, 5, 7]';        %The semitone frequency multiplier is 2^(1/12)
    end
    tunes(i, 1:end) = tunes(6, 1:end) .* frequency_diff(i);
end
% tunes=%[
% 175    349    698      0
% 196    392    784      0
...
padding = 1;
last_padding = 0;                                                   %Record the length of the overlap of a note(Sampling points)
...
audiowrite('music_02.wav', tone_sampling, frequency_sampling);      %store .wav
